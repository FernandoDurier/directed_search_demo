...
def init_screen(n_rows, n_cols):
    ...
    pygame.display.set_caption("Labyrinth Dijkstra's Pathfinding")
    return screen
...
# Dijkstra's algorithm with visual feedback
def dijkstra_search_with_ui(labyrinth, start_pos, exit_pos, screen):
    ...
    while open_list:
        ...
        for direction, neighbor_id in connections.items():
            if neighbor_id is not None:
                ...
                if not labyrinth[neighbor_row][neighbor_col]["wall"]:
                    distance = current_distance + 1  # Assuming uniform cost for movement
...
def save_screen_as_jpg(screen):
    ...
    filename = f"dijkstra_labyrinth_{timestamp}.jpg"
...
def main():
    ...
    # Run Dijkstra's pathfinding
    path = dijkstra_search_with_ui(labyrinth, start_pos, exit_pos, screen)
    if path is not None:
