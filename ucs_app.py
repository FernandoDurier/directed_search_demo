...
# Uniform Cost Search with visual feedback
def uniform_cost_search_with_ui(labyrinth, start_pos, exit_pos, screen):
    n_rows, n_cols = len(labyrinth), len(labyrinth[0])
    open_list = []
    heapq.heappush(open_list, (0, start_pos, []))  # (g_cost, current_position, path_taken)
            ...
            if neighbor_id is not None:
                ...
                if (neighbor_row, neighbor_col) not in visited and not labyrinth[neighbor_row][neighbor_col]["wall"]:
                    ...
                    if (neighbor_row, neighbor_col) not in g_costs or g_cost < g_costs[(neighbor_row, neighbor_col)]:
                        ...
                        heapq.heappush(open_list, (g_cost, (neighbor_row, neighbor_col), new_path))
...
def main():
    ...
    path = uniform_cost_search_with_ui(labyrinth, start_pos, exit_pos, screen)
